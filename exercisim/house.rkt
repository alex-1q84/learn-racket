...
(define objects '(("the house that Jack built." "lay in")
                  ("the malt" "ate")
                  ("the rat" "killed")
                  ("the cat" "worried")
                  ("the dog" "tossed")
                  ...
                  ("the horse and the hound and the horn" false)))

(define (house [start 1] [end 12])
  (string-join
   (for/list ([i (in-range start (add1 end))])
     (list->nursery-rhyme (take objects i)))
   "\n\n"))

(define (list->nursery-rhyme objects)
  (define sentences
    (let loop ([current (car objects)]
      ...
      (cond
        [(empty? rest)
         (cons (string-append "This is " (car current)) sentences)]
        [else
         (loop (car rest)
               (cdr rest)
               (cons (string-append "that " (cadr current) " " (car current)) sentences))])))
