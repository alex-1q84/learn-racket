...
(define (proteins strand)
  (define codons
    (for/list ([n (in-range 0 (string-length strand) 3)])
      (let ([codon (substring strand n (min (+ n 3) (string-length strand)))])
        (for ([ch (in-string codon)])
          (when ((negate member)  ch '(#\U #\C #\G #\A))
              (error "Unknown amino acids, not part of a codon, can't translate")))
        codon)))
  
  (for/list ([codon (in-list codons)]
    ...
    (case codon
      ...
      [("UGU" "UGC") "Cysteine"]
      [("UGG") "Tryptophan"]
      [else (error "Non-existing codon can't translate")])))
