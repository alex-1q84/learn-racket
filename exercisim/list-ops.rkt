...
(define (my-length lst)
  (my-fold (lambda (x base) (add1 base))
           0 lst))
...
(define (my-map f lst)
  (my-reverse (my-fold (lambda (x base) (cons (f x) base))
                       '() lst)))

(define (my-filter f lst)
  (my-reverse (my-fold (lambda (x base)
                         (if (f x) (cons x base)
                             base))
                       '()
                       lst)))

(define (my-fold f initial lst)
  (cond
    [(empty? lst) initial]
    [else (my-fold f (f (car lst) initial) (cdr lst))]))

(define (my-append alst blst)
  (my-fold cons blst (my-reverse alst)))

(define (my-concatenate lsts)
  (my-fold my-append '() (my-reverse lsts)))
