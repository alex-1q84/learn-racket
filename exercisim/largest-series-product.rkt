...
(define (windows size step lst)
  (let ([init (take lst size)]
        [tail (drop lst size)])
    (for/fold ([init init] [result (list init)] #:result result) ([c (in-list tail)])
      (values (append (cdr init) (list c))
              (append result (list (append (cdr init) (list c))))))))

(define (largest-product digits span)
  (if (zero? span)
      1
      (let ([series (windows span 1 (map string->number
                                         (map string (string->list digits))))])
        (apply max (map (curry apply *) series)))))
